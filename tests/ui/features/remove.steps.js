...
Then(/^The first contact is removed$/, function (callback) {
    this.browser.visit(" http://127.0.0.1:3000", (err) => {
        ...
        var contact = this.browser.tabs.current.Contact.Contacts.instance();
        var bouton = this.browser.queryAll('table tbody tr td a')[0];
        ...
        this.browser.assert.success(size !== size2 && prenom.innerHTML !== prenom2.innerHTML && nom.innerHTML !== nom2.innerHTML);
        callback();
    });
});